...
```{r}
return_pv<-replicate(10, calc_pv(surv_mat, fert_mat, N0_data, nstage, years=1:10, stage2mod=2, beta=0.01))

single_pv<-calc_pv(surv_mat, fert_mat, N0_data, nstage, years=1:10, stage2mod=2, beta=0.01)
```
...
demog<-surv_lessthan_alpha(survival_pv, alpha)
n_pv<-abundance_lessthan_alpha(abundance_pv,alpha)

diff_mean<-mean(demog-n_pv)
delta<-0.01
power<-diff_mean*delta

```{r, echo=FALSE}
plot(demog~alpha, type='l', col='blue', xlab='freq p<alpha', ylab='alpha', main='Frequence p is less than alpha')
lines(n_pv~alpha, col='red', type='l')
```
