...
CiPi <- subset(mydata, POP =="CIPI_1")

##pulling out cipi survival matrices
surv <- array(unlist(CiPi), dim = c(nrow(CiPi[[1]]$matU), ncol(CiPi[[1]]$matU), length(CiPi)))

##pulling out cipi reproduction
fert <- array(unlist(CiPi), dim = c(nrow(CiPi[[1]]$matF), ncol(CiPi[[1]]$matF), length(CiPi)))

##pulling out cipi transition matrices
trans <- array(unlist(CiPi), dim = c(nrow(CiPi[[1]]$matA), ncol(CiPi[[1]]$matA), length(CiPi)))
...
surv_trend <- trend(surv_rand, beta = 0.1, nstage, stage2mod=2)
